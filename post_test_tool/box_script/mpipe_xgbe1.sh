...
console_save=$6
tmp_detail="/tmp/$1-$2-xgbe01-detail"
...
touch $1-$2.lock
echo -e "[Test-Module]: `date +'%Y-%m-%d %T'` MPIPE-XGBE01            \t0/$4" >> $console_save
rm -rf $1-$2.lock 
...
taskset -c 21,22 napa_mpipe_aging --interface xgbe1 --worker 1 2>&1 | tee -a $tmp_detail >> $3 &
...
do
	...
	then
			...
			error="[error]"
        	sed -ie "/MPIPE-XGBE01/c\[Test-Module]: `date +'%Y-%m-%d %T'` MPIPE-XGBE01            `echo -ne "\t\033[0;31;1m${failed}${error}\033[0m"`/$4    ${mpipe_errors}        error" $console_save
        	rm -rf $1-$2.lock
#			kill -9 $5 > /dev/null 2>&1
	...
	then
		...
		then
			...
			if [ "$mpipe_errors" == "0-0" ]
			then
        		sed -ie "/MPIPE-XGBE01/c\[Test-Module]: `date +'%Y-%m-%d %T'` MPIPE-XGBE01             `echo -ne "\t\033[0;32;1m$success\033[0m"`/$4    ${mpipe_errors}" $console_save
			else
				sed -ie "/MPIPE-XGBE01/c\[Test-Module]: `date +'%Y-%m-%d %T'` MPIPE-XGBE01             `echo -ne "\t\033[0;31;1m$success\033[0m"`/$4    ${mpipe_errors}        error" $console_save
			fi
			rm -rf $1-$2.lock
...
#sed -ie "/MPIPE-XGBE01/c\[Test-Module]: `date +'%Y-%m-%d %T'` MPIPE-XGBE01     `echo -ne "\t\033[0;32;1mFinished\033[0m"`/$4" $console_save
